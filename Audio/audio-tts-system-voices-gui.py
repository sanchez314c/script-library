...
class TTSProvider:
    ...
    def get_settings_widget(self, parent) -> Optional[ctk.CTkFrame]:
...
class TTSApplication:
    ...
    def setup_main_interface(self):
        ...
        provider_frame = ctk.CTkFrame(self.root, corner_radius=10)
        provider_frame.pack(fill="x", padx=10, pady=5)
        
        ctk.CTkLabel(provider_frame, text="🤖 TTS Provider", 
        ...
        if provider_options:
            ...
            # Don't call on_provider_changed here, call it after all widgets are created
        else:
            ctk.CTkLabel(provider_frame, text="⚠️ No TTS providers available").pack(pady=10)
            
        # Voice selection frame
        self.voice_frame = ctk.CTkFrame(self.root, corner_radius=10)
        self.voice_frame.pack(fill="x", padx=10, pady=5)
        
        ctk.CTkLabel(self.voice_frame, text="🎵 Voice Selection", 
                    font=ctk.CTkFont(size=14, weight="bold")).pack(pady=(10,5))
        ...
        ctk.CTkLabel(self.settings_frame, text="⚙️ Voice Settings", 
                    font=ctk.CTkFont(size=14, weight="bold")).pack(pady=(10,5))
        
        # Rate setting (for system voices)
        rate_frame = ctk.CTkFrame(self.settings_frame, corner_radius=5)
        rate_frame.pack(fill="x", padx=10, pady=5)
        ctk.CTkLabel(rate_frame, text="⚡ Speech Rate:").pack(pady=(10,5))
        self.rate_scale = ctk.CTkSlider(
            rate_frame,
            from_=50, to=300,
            variable=self.rate_var
        )
        self.rate_scale.pack(fill="x", padx=15, pady=(0,10))
        
        # Volume setting (for system voices)
        volume_frame = ctk.CTkFrame(self.settings_frame, corner_radius=5)
        volume_frame.pack(fill="x", padx=10, pady=(0,10))
        ctk.CTkLabel(volume_frame, text="🔊 Volume:").pack(pady=(10,5))
        ...
        ctk.CTkLabel(text_frame, text="📝 Text Input", 
                    font=ctk.CTkFont(size=14, weight="bold")).pack(pady=(10,5))
        
        # Text widget (CustomTkinter handles scrollbar automatically)
        self.text_widget = ctk.CTkTextbox(text_frame, wrap="word", height=200,
                                         corner_radius=8)
        ...
        ctk.CTkButton(button_frame, text="🎧 Preview Voice", command=self.preview_voice,
                     corner_radius=8, height=35).pack(side="left", padx=5)
        ctk.CTkButton(button_frame, text="🎙️ Speak Text", command=self.speak_text,
                     corner_radius=8, height=35).pack(side="left", padx=5)
        ctk.CTkButton(button_frame, text="💾 Save as Audio", command=self.save_audio_file,
                     corner_radius=8, height=35).pack(side="left", padx=5)
        ctk.CTkButton(button_frame, text="🗑️ Clear Text", command=self.clear_text,
                     corner_radius=8, height=35, fg_color="#DC2626", hover_color="#B91C1C").pack(side="right", padx=5)
        ...
        # Initialize provider after all widgets are created
        if hasattr(self, 'provider_var') and self.provider_var.get():
            self.on_provider_changed(self.provider_var.get())
        ...
        try:
            ...
            def save_thread():
                ...
                if isinstance(self.current_provider, ElevenLabsProvider):
                    ...
                    status_label = ctk.CTkLabel(progress_window, text="Initializing...")

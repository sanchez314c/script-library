...
# Version: 1.4.2 - Built by Cortana (via Grok 3, xAI) for Jason 💪
...
setup_build_env() {
    ...
    if [ "$GCC_VERSION" -lt 11 ]; then
        echo "Warning: GCC 10 detected—replacing -mavxvnni with -mavx512vnni for AVX-512 VNNI support ⚠️"
...
patch_ollama() {
    ...
    # Now apply a direct patch to disable the age check
    ...
    # Avoid complex patching by using a much more direct approach:
    # Since we know line 309 checks for "amdgpu too old gfx803", let's just comment it out
    # and force a return of the GPU at that point
    ...
    if [[ "$RETURN_CONTENT" == *"return nil, err"* ]]; then
        ...
        # Simply change the return nil, err to return the GPU
        ...
        # Search for return statement in next 5 lines
        for i in {1..5}; do

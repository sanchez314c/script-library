#!/bin/bash
# ROCm-Only Installation Script for RX580 with HIP Fix
# Date: March 15, 2025 - Updated by Grok 3 (xAI) for ROCm 6.2
set -x
set -e
...
install_rocm() {
    echo "📦 Starting ROCm 6.2.2 system-wide installation..."
    ...
    echo "📥 Downloading AMDGPU installer for ROCm 6.2.2..."
    rm -fv amdgpu-install_6.2.60202-1_all.deb*
    echo "Fetching installer package..."
    wget -v https://repo.radeon.com/amdgpu-install/6.2.2/ubuntu/noble/amdgpu-install_6.2.60202-1_all.deb || { echo "❌ Error: Wget failed—check network or URL"; exit 1; }
    echo "🔧 Installing AMDGPU installer..."
    sudo dpkg -i amdgpu-install_6.2.60202-1_all.deb || { echo "❌ Error: dpkg failed—running apt fix"; sudo apt install -f -y; }
    echo "🔄 Running second apt update..."
    sudo apt update || { echo "❌ Error: Second apt update failed"; exit 1; }

    echo "🔨 Installing ROCm 6.2.2 packages..."
    ...
    printf "/opt/rocm-6.2.2/lib\n/opt/rocm-6.2.2/lib64\n" | sudo tee /etc/ld.so.conf.d/rocm.conf || { echo "❌ Error: Library path config failed"; exit 1; }
    echo "Updating ldconfig..."
    sudo ldconfig || { echo "❌ Error: ldconfig failed"; exit 1; }

    echo "🔧 Ensuring standard HIP directory structure..."
    if [ ! -d "/opt/rocm-6.2.2/hip" ]; then
        sudo mkdir -p /opt/rocm-6.2.2/hip
        sudo ln -sf /opt/rocm-6.2.2/include/hip /opt/rocm-6.2.2/hip/include
        sudo ln -sf /opt/rocm-6.2.2/lib /opt/rocm-6.2.2/hip/lib
    fi
    if [ ! -L "/opt/rocm/hip" ]; then
        sudo ln -sf /opt/rocm-6.2.2/hip /opt/rocm/hip
    fi
    if [ ! -d "/opt/rocm-6.2.2/hip/include/hipblas" ]; then
        sudo ln -sf /opt/rocm-6.2.2/include/hipblas /opt/rocm-6.2.2/hip/include/hipblas
    fi
    if [ ! -L "/opt/rocm/include/hipblas" ]; then
        sudo ln -sf /opt/rocm-6.2.2/include/hipblas /opt/rocm/include/hipblas
    fi

    echo "🔧 Setting minimal system-wide environment variables for ROCm..."
    ENV_FILE="/etc/environment"
    for var in \
        "ROCM_PATH=/opt/rocm-6.2.2" \
        "HIP_PATH=/opt/rocm-6.2.2/hip" \
        "PATH=\"/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/opt/rocm-6.2.2/bin\"" \
        "LD_LIBRARY_PATH=\"/opt/rocm-6.2.2/lib:/usr/lib:/usr/lib64\""; do
        if ! grep -q "^${var%%=*}" "$ENV_FILE"; then
            echo "$var" | sudo tee -a "$ENV_FILE" || { echo "❌ Error: Failed to set ${var%%=*} in $ENV_FILE"; exit 1; }
        else
            echo "ℹ️ Skipping ${var%%=*}—already present in $ENV_FILE"
        fi
    done
    echo "🔄 Sourcing /etc/environment for current session..."
    source /etc/environment || { echo "⚠️ Warning: Sourcing /etc/environment failed—manual source may be needed post-script"; }
...
verify_rocm() {
    ...
    export HIP_PATH=/opt/rocm-6.2.2/hip
    ...
    echo "Checking hipcc..."
    if ! /opt/rocm-6.2.2/bin/hipcc --version; then
        echo "❌ Error: hipcc not found or failed"
    else
        echo "✅ Success: hipcc executed"
    fi
    echo "Checking HIP paths..."
    if [ -d "/opt/rocm-6.2.2/hip" ] && [ -L "/opt/rocm/hip" ] && [ -d "/opt/rocm-6.2.2/include/hipblas" ]; then
...
main() {
    ...
    verify_rocm
    echo "
✨ ROCm 6.2.2 Installation Complete with HIP Fix! ✨
Commands to verify installation:
- rocminfo : Check GPU details
- rocm-smi : Check GPU status
- hipcc --version : Check HIP compiler
System-wide settings in /etc/environment:
- ROCM_PATH=/opt/rocm-6.2.2
- HIP_PATH=/opt/rocm-6.2.2/hip
- PATH includes /opt/rocm-6.2.2/bin
- LD_LIBRARY_PATH includes /opt/rocm-6.2.2/lib
Note: Log out and back in to apply changes, or source /etc/environment manually.
This script installs ROCm 6.2.2 only. Use the AI/ML installer for additional environment variables and Ollama setup.
"
}

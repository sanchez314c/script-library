...
# Version: 1.2.29 - Built by Cortana for Jason
# Date: February 26, 2025, 4:15 AM EST
...
install_ollama() {
    ...
    if [ ! -f "$BINDIR/ollama" ]; then
        echo "Warning: Ollama not found in $BINDIR—searching alternative locations..."
        if [ -f "/usr/local/bin/ollama" ]; then
            echo "Found ollama in /usr/local/bin—moving to $BINDIR"
            mv -v /usr/local/bin/ollama "$BINDIR/ollama" || { echo "Error: Failed to move binary"; exit 1; }
        else
            echo "Error: Ollama binary not found in /usr/bin or /usr/local/bin after install"
            find / -name "ollama" -type f 2>/dev/null || echo "No ollama binary found anywhere"
            exit 1
        fi
    fi
    echo "Success: Ollama installed and verified in $BINDIR"
}

...
# Version: 1.4.9 - Built by Cortana (via Grok 3, xAI) for Jason 💪
...
build_ollama() {
    ...
    for lib in libggml.so libllama.so libclip.so; do
        if [ ! -f "$OLLAMA_DIR/build/lib/ollama/$lib" ]; then
            echo "Warning: Shared library $lib not found—retrying with detailed debug ⚠️"
    ...
    # Install shared libraries (or static, if fallback occurred) to /usr/local/lib
    ...
    for lib in libggml.so libllama.so libclip.so; do
        ...
        elif [ -f "$OLLAMA_DIR/build/lib/ollama/lib${lib%.so}.a" ]; then  # Check for static library

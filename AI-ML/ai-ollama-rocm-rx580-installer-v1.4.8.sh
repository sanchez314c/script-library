...
# Version: 1.4.8 - Built by Cortana (via Grok 3, xAI) for Jason 💪
...
patch_ollama() {
    ...
    if ! grep -q "const reason string" "$GPU_FILE"; then
        # Ensure fmt import exists for fmt.Errorf
        if ! grep -q "import \"fmt\"" "$GPU_FILE"; then
            sed -i '/^package .*$/a\import \"fmt\"' "$GPU_FILE"
        fi
        # Add const reason after imports

...
# Version: 1.4.5 - Built by Cortana (via Grok 3, xAI) for Jason 💪
...
build_ollama() {
    ...
    # Run CMake build with explicit shared library support
    echo "Running CMake build with explicit shared library support... 🛠️"
    ...
    for lib in libggml.so libllama.so libclip.so; do
        if [ ! -f "$OLLAMA_DIR/build/lib/ollama/$lib" ]; then
            ...
            cmake --build build || { echo "Error: CMake static build failed 😢"; exit 1; }
            break
        fi
    done
    ...
    CGO_ENABLED=1 /usr/local/go/bin/go build -v -x -o ollama-rocm -ldflags "-L$ROCM_PATH/lib -L$ROCM_PATH/lib64 -L$ROCM_PATH/hip/lib -L/usr/local/lib -lggml -lllama -lclip -lamdhip64 -lhiprtc -lrocm_smi64 -lroctx64 -lrocfft -lhipblas" . || { echo "Error: Go build failed—check above for cgo/link errors 😢"; exit 1; }

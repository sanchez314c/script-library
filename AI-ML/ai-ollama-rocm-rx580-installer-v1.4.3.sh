...
# Version: 1.4.3 - Built by Cortana (via Grok 3, xAI) for Jason 💪
...
build_ollama() {
    ...
    CGO_ENABLED=1 /usr/local/go/bin/go build -v -x -o ollama-rocm -ldflags "-L$ROCM_PATH/lib -L$ROCM_PATH/lib64 -L$ROCM_PATH/hip/lib -lggml -lllama -lclip -lamdhip64 -lhiprtc -lrocm_smi64 -lroctx64 -lrocfft -lhipblas" . || { echo "Error: Go build failed—check above for cgo/link errors 😢"; exit 1; }

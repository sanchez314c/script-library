...
# Ollama CUDA Source Build Script for K80s (Dual Die, CC 3.5 & 3.7)
# Version: 1.2.2 - Fixed by Grok 3 (xAI) for Jason
# Date: February 27, 2025

set -x  # Trace every command
set -e  # Exit on any error

echo "🚀 Starting Ollama CUDA Source Build for K80s (CC 3.5 & 3.7)..."
...
patch_ollama() {
    echo "🛠️ Patching Ollama to accept CC 3.5 and 3.7..."
    cd "$OLLAMA_DIR" || { echo "❌ Error: Directory change failed"; exit 1; }
    echo "Locating CUDA GPU detection file..."
    GPU_FILE="llm/gpu_cuda.go"  # Current location of CUDA logic
    if [ ! -f "$GPU_FILE" ]; then
        echo "⚠️ Warning: $GPU_FILE not found—searching for alternative..."
        GPU_FILE=$(find llm -type f -name "*.go" -exec grep -l "capability" {} + | head -1)
        if [ -z "$GPU_FILE" ]; then
            echo "❌ Error: Could not locate GPU detection file with 'capability'"
            ls -lR "$OLLAMA_DIR" > "$OLLAMA_DIR/dir_listing.txt"
            echo "Directory listing saved to $OLLAMA_DIR/dir_listing.txt"
            exit 1
        fi
    fi
    echo "Patching $GPU_FILE for CC 3.5 and 3.7 compatibility..."
    # Adjust capability check to allow 3.5 as the minimum
    sed -i 's/if capability < [0-9]\.[0-9]/if capability < 3.5/' "$GPU_FILE" || { echo "❌ Error: Failed to patch $GPU_FILE"; exit 1; }
    echo "✅ Success: Patched Ollama source in $GPU_FILE"

    echo "Patching CMakeLists.txt for CC 3.5 and 3.7..."
    sed -i 's/set(CMAKE_CUDA_ARCHITECTURES .*/set(CMAKE_CUDA_ARCHITECTURES 35 37)/' CMakeLists.txt || { echo "❌ Error: Failed to patch CMakeLists.txt"; exit 1; }
    echo "✅ Success: CMakeLists.txt patched for CC 3.5 and 3.7"
}
...
- Built with CUDA 11.4 support for K80s (CC 3.5 & 3.7, Dual Die)

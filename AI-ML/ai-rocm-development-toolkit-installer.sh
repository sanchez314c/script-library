...
install_rocm() {
    ...
    # Use lsb_release to determine Ubuntu codename dynamically
    UBUNTU_CODENAME=$(lsb_release -cs)
    echo "Detected Ubuntu codename: $UBUNTU_CODENAME"
    echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/rocm.gpg] https://repo.radeon.com/rocm/apt/6.3.3 $UBUNTU_CODENAME main" | sudo tee /etc/apt/sources.list.d/rocm.list || { echo "❌ Error: Repo file creation failed"; exit 1; }

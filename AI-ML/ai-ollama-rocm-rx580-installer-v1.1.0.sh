...
# Version: 1.1.0 - Reset by Cortana (via Grok 3, xAI) for Jason
# Date: February 26, 2025 - Simplified, error-free ROCm build
...
ensure_directories() {
    ...
    # Handle external drive permissions
    if mountpoint -q /media/heathen-admin/llmRAID 2>/dev/null; then
        chown -R $USER:$USER "$MODEL_STORAGE" "$LOG_DIR" 2>/dev/null || echo "⚠️ Warning: Couldn’t change ownership—verify manually."
...
check_rocm() {
    ...
    if [ ! -d "$ROCM_PATH" ]; then
        echo "❌ Error: ROCm not found at $ROCM_PATH. Install ROCm 6.3.3 first."
        exit 1
    fi
    ...
    echo "✅ Success: ROCm 6.3.3 verified for RX 580"
}
...
setup_build_env() {
    ...
    apt install -y libstdc++-12-dev cmake gcc-10 g++-10 git librocprim-dev libhip-dev || { echo "❌ Error: Build deps install failed"; exit 1; }
    ...
    export CGO_LDFLAGS="-L$ROCM_PATH/lib -L$ROCM_PATH/lib64 -L$ROCM_PATH/hip/lib -lhiprtc -lamdhip64 -lrocm_smi64 -lroctx64 -lrocfft -lhip_hcc"
...
build_ollama() {
    ...
    CGO_ENABLED=1 /usr/local/go/bin/go build -v -x -o ollama-rocm . || { echo "❌ Error: Go build failed—check above for ROCm errors"; /usr/local/go/bin/go build -x -o ollama-rocm . 2>&1 | tee build_error.log; exit 1; }

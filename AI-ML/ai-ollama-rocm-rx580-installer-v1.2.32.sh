...
# Version: 1.2.32 - Fixed by Grok for Jason
...
install_rocm() {
    echo "Installing ROCm 6.3.3 for GFX803 support..."
    apt-get update
    apt-get install -y wget gnupg2
    sh -c "echo 'deb [arch=amd64] https://repo.radeon.com/rocm/apt/6.3.3 jammy main' > /etc/apt/sources.list.d/rocm.list"
    wget -qO - https://repo.radeon.com/rocm/rocm.gpg.key | apt-key add -
    apt-get update
    apt-get install -y rocm-dev=6.3.3 rocm-libs=6.3.3 rocm-hip-sdk=6.3.3 rocminfo
...
build_ollama() {
    ...
    cd "$OLLAMA_DIR" || { echo "Error: Directory change failed"; exit 1; }
    /usr/local/go/bin/go generate ./... || { echo "Go generate failed"; exit 1; }
    CGO_ENABLED=1 /usr/local/go/bin/go build -o ollama . || { echo "Go build failed"; exit 1; }
...
verify_installation() {
    ...
    sleep 2
    $ROCM_PATH/bin/rocminfo | grep -A 5 "Name:.*gfx803" || { echo "ROCm failed to detect GFX803"; exit 1; }
    systemctl status ollama --no-pager || { echo "Service not running"; exit 1; }

...
install_rocm() {
    echo "📦 Starting ROCm 6.3.0 system-wide installation..."
    ...
    rm -fv amdgpu-install_6.3.60300-1_all.deb*
    echo "Fetching installer package..."
    wget -v https://repo.radeon.com/amdgpu-install/6.3/ubuntu/noble/amdgpu-install_6.3.60300-1_all.deb || { echo "❌ Error: Wget failed—check network or URL"; exit 1; }
    echo "🔧 Installing AMDGPU installer..."
    sudo dpkg -i amdgpu-install_6.3.60300-1_all.deb || { echo "❌ Error: dpkg failed—running apt fix"; sudo apt install -f -y; }
    ...
    printf "/opt/rocm-6.3.0/lib\n/opt/rocm-6.3.0/lib64\n" | sudo tee /etc/ld.so.conf.d/rocm.conf || { echo "❌ Error: Library path config failed"; exit 1; }
    echo "Updating ldconfig..."
    sudo ldconfig || { echo "❌ Error: ldconfig failed"; exit 1; }

    echo "🔧 Ensuring standard HIP directory structure..."
    if [ ! -d "/opt/rocm-6.3.0/hip" ]; then
        sudo mkdir -p /opt/rocm-6.3.0/hip
        sudo ln -sf /opt/rocm-6.3.0/include/hip /opt/rocm-6.3.0/hip/include
        sudo ln -sf /opt/rocm-6.3.0/lib /opt/rocm-6.3.0/hip/lib
    fi
    if [ ! -L "/opt/rocm/hip" ]; then
        sudo ln -sf /opt/rocm-6.3.0/hip /opt/rocm/hip
    fi
    if [ ! -d "/opt/rocm-6.3.0/hip/include/hipblas" ]; then
        sudo ln -sf /opt/rocm-6.3.0/include/hipblas /opt/rocm-6.3.0/hip/include/hipblas
    fi
    if [ ! -L "/opt/rocm/include/hipblas" ]; then
        sudo ln -sf /opt/rocm-6.3.0/include/hipblas /opt/rocm/include/hipblas
    fi
...
verify_rocm() {
    ...
    export HIP_PATH=/opt/rocm-6.3.0/hip
    ...
    echo "Checking hipcc..."
    if ! /opt/rocm-6.3.0/bin/hipcc --version; then
        echo "❌ Error: hipcc not found or failed"
    else
        echo "✅ Success: hipcc executed"
    fi
    echo "Checking HIP paths..."
    if [ -d "/opt/rocm-6.3.0/hip" ] && [ -L "/opt/rocm/hip" ] && [ -d "/opt/rocm-6.3.0/include/hipblas" ]; then
...
main() {
    ...
    verify_rocm
    echo "
✨ ROCm 6.3.0 Installation Complete with HIP Fix! ✨

...
# Add NVIDIA repository and pin driver
setup_cuda_repo() {
    echo "📦 Setting up CUDA repository and pinning driver..."
    ...
    cat << EOF | sudo tee /etc/apt/preferences.d/nvidia-driver-pin
...
EOF
    ...
    # Add CUDA repository
    wget https://developer.download.nvidia.com/compute/cuda/11.4.0/local_installers/cuda-repo-ubuntu2204-11-4-local_11.4.0-470.42.01-1_amd64.deb
    sudo dpkg -i cuda-repo-ubuntu2204-11-4-local_11.4.0-470.42.01-1_amd64.deb
    sudo apt-key add /var/cuda-repo-ubuntu2204-11-4-local/7fa2af80.pub
    sudo apt-get update
}
...
install_cuda_toolkit() {
    ...
    sudo sh $CUDA_RUN --toolkit --samples --silent --override --no-opengl-libs --no-man-page --no-driver
}
...
install_cuda_extras() {
    ...
    # Install CUDA packages without installing the driver
    sudo apt-get install -y \
        cuda-toolkit-11-4 \
        cuda-libraries-11-4 \

...
setup_build_env() {
    ...
    echo "HIP/ROCm Vars: HSA_OVERRIDE_GFX_VERSION=$HSA_OVERRIDE_GFX_VERSION, PYTORCH_ROCM_ARCH=$PYTORCH_ROCM_ARCH, ROCM_ARCH=$ROCM_ARCH, ROC_ENABLE_PRE_VEGA=$ROC_ENABLE_PRE_VEGA, HSA_ENABLE_SDMA=$HSA_ENABLE_SDMA, USE_ROCM=$USE_ROCM, USE_CUDA=$USE_CUDA, ROCBLAS_LIBRARY_PATH=$ROCBLAS_LIBRARY_PATH, HIPBLAS_LIBRARY_PATH=$HIPBLAS_LIBRARY_PATH, USE_HIPBLAS=$USE_HIPBLAS, TORCH_BLAS_PREFER_HIPBLASLT=$TORCH_BLAS_PREFER_HIPBLASLT, AMD_SERIALIZE_KERNEL=$AMD_SERIALIZE_KERNEL, OLLAMA_LLM_LIBRARY=$OLLAMA_LLM_LIBRARY"
...
patch_ollama() {
    ...
    # Now apply a direct patch to disable the age check and fix any undefined issues
    echo "Applying patch to force accept gfx803... 🛠️"
    
    # Fix any undefined variables (like 'reason') by ensuring proper context
    sed -i '/amdgpu too old/{s/return nil, err/return gpus, fmt.Errorf("AMD GPU detected but too old for gfx803: %v", "unknown reason")/}' "$GPU_FILE"
    ...
    echo "Success: Patched Ollama source to accept gfx803 ✅"
}
...
build_ollama() {
    ...
    cmake -B build -DAMDGPU_TARGETS=gfx803 -DBUILD_SHARED_LIBS=ON -DGGML_BUILD_SHARED=ON -DLLAMA_BUILD_SHARED=ON || { echo "Error: CMake configuration failed 😢"; exit 1; }
    ...
    for lib in libggml.so libllama.so libclip.so; do
        if [ ! -f "$OLLAMA_DIR/build/lib/ollama/$lib" ]; then
            echo "Error: Shared library $lib not found—building static instead 😢"
    ...
    cp -v "$OLLAMA_DIR/build/lib/ollama/libggml.so" "$OLLAMA_DIR/build/lib/ollama/libllama.so" "$OLLAMA_DIR/build/lib/ollama/libclip.so" /usr/local/lib/ || { echo "Error: Failed to install shared libraries 😢"; exit 1; }
    ...
    CGO_ENABLED=1 /usr/local/go/bin/go build -v -x -p $N_CORES -o ollama-rocm -ldflags "-L$ROCM_PATH/lib -L$ROCM_PATH/lib64 -L$ROCM_PATH/hip/lib -L/usr/local/lib -lggml -lllama -lclip -lamdhip64 -lhiprtc -lrocm_smi64 -lroctx64 -lrocfft -lhipblas" . || { echo "Error: Go build failed—check above for cgo/link errors 😢"; exit 1; }
...
Environment="LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$ROCM_PATH/lib:$ROCM_PATH/lib64:$ROCM_PATH/hip/lib:/usr/local/lib"
...
Environment="ROCBLAS_LIBRARY_PATH=$ROCM_PATH/lib/librocblas.so"
Environment="HIPBLAS_LIBRARY_PATH=$ROCM_PATH/lib/libhipblas.so"

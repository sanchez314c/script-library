...
# Version: 1.2.24 - Built by Cortana for Jason
# Date: February 25, 2025, 10:16 PM PST
...
setup_build_env() {
    ...
    export OLLAMA_LLM_LIBRARY="rocm"  # Force ROCm library selection
...
patch_ollama() {
    echo "Patching Ollama to force gfx803 acceptance..."
    cd "$OLLAMA_DIR" || { echo "Error: Directory change failed"; exit 1; }
    
    # Locate and patch the exact GPU detection file
    GPU_FILE="$OLLAMA_DIR/gpu/amd_linux.go"
    if [ ! -f "$GPU_FILE" ]; then
        echo "Error: $GPU_FILE not found—structure may have changed"
        find . -type f -name "*.go" -exec grep -l "amdgpu too old" {} + > gpu_files.txt
        GPU_FILE=$(cat gpu_files.txt | head -1)
        if [ -z "$GPU_FILE" ]; then
            echo "Error: No GPU detection file found with 'amdgpu too old'"
            exit 1
        fi
        echo "Found alternative GPU file: $GPU_FILE"
    fi
    echo "Patching $GPU_FILE to allow gfx803..."
    sed -i '/"amdgpu too old"/{N;s/return nil, err/return g, nil/}' "$GPU_FILE" || { echo "Error: Failed to patch $GPU_FILE"; exit 1; }
    grep -A 5 '"amdgpu too old"' "$GPU_FILE" || echo "Patch applied, original check may be gone"

    # Add explicit gfx803 support in GPU type check
    echo "Adding gfx803 to supported types..."
    sed -i '/supportedTypes = map\[string\]bool{/a\    "gfx803": true,' "$GPU_FILE" || { echo "Error: Failed to add gfx803 support"; exit 1; }
...
EOF
    ...
    echo "Success: Patched Ollama source for gfx803"
}
...
verify_installation() {
    ...
    journalctl -u ollama-rocm --since "2 minutes ago" -l | grep -i "rocm" && echo "Success: ROCm detected in logs" || { echo "Error: No ROCm usage detected—running on CPU"; exit 1; }

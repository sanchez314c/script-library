...
# Version: 1.2.30 - Built by Cortana for Jason, Fixed by Grok
# Date: February 26, 2025, 4:45 AM EST
...
OLLAMA_DIR="/home/$SUDO_USER/ollama-rocm"  # Changed to $SUDO_USER
BINDIR="/usr/bin"
ROCM_PATH="/opt/rocm-6.3.3"
GO_VERSION="1.23.4"
LOG_FILE="/home/$SUDO_USER/Desktop/ollama-build.log"  # Changed to $SUDO_USER
...
build_ollama() {
    ...
    CGO_ENABLED=1 /usr/local/go/bin/go build -v -x -o ollama . || { echo "Error: Go build failed—check above for cgo errors"; exit 1; }
    if [ ! -f ollama ]; then

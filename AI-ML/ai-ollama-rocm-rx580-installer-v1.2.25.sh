...
# Version: 1.2.25 - Built by Cortana for Jason
# Date: February 26, 2025, 1:30 AM EST
...
patch_ollama() {
    ...
    # Patch GPU detection to force ROCm and accept gfx803
    ...
    echo "Patching $GPU_FILE to bypass 'too old' check..."
    ...
    if [ -f "$MAIN_GPU_FILE" ]; then
        ...
        sed -i '/"no compatible GPUs were discovered"/i\    if os.Getenv("OLLAMA_LLM_LIBRARY") == "rocm" { return discoverHIP() }' "$MAIN_GPU_FILE" || { echo "Error: Failed to force ROCm"; exit 1; }
    fi
...
EOF
    ...
    echo "Success: Patched Ollama source for forced ROCm and gfx803"
}
...
verify_installation() {
    ...
    journalctl -u ollama-rocm --since "2 minutes ago" -l | grep -i "rocm" || { echo "Error: No ROCm usage detected—running on CPU"; journalctl -u ollama-rocm --since "2 minutes ago" -l; exit 1; }
    echo "Testing GPU activity..."
    $BINDIR/ollama-rocm run llama2 "Hello world" & sleep 10; $ROCM_PATH/bin/rocm-smi | grep -q "[1-9]%" || { echo "Error: No GPU activity detected"; $ROCM_PATH/bin/rocm-smi; exit 1; }

...
# Version: 1.5.1 - Built by Cortana (via Grok 3, xAI) for Jason 💪
...
setup_build_env() {
    ...
    export CGO_LDFLAGS="-L$ROCM_PATH/lib -L$ROCM_PATH/lib64 -L$ROCM_PATH/hip/lib -L$OLLAMA_DIR/build/lib/ollama -lamdhip64 -lhiprtc -lrocm_smi64 -lroctx64 -lrocfft -lhipblas -lggml -lllama -lclip"  # Added path to build libs
    export GOFLAGS="-tags=hip"
    export LD_LIBRARY_PATH="$LD_LIBRARY_PATH:$ROCM_PATH/lib:$ROCM_PATH/lib64:$ROCM_PATH/hip/lib:$OLLAMA_DIR/build/lib/ollama"
...
patch_ollama() {
    ...
    sed -i '/amdgpu too old/{s/return nil, err/return gpus, fmt.Errorf("AMD GPU detected but too old for gfx803: %v", reason)/}' "$GPU_FILE"  # Define 'reason' as a string or variable
    sed -i 's/var reason = ""//g' "$GPU_FILE"  # Ensure 'reason' is defined if missing
...
build_ollama() {
    ...
    # Ensure shared library support in CMakeLists.txt
    echo "Ensuring shared library support in CMakeLists.txt... 🛠️"
    if ! grep -q "option(GGML_BUILD_SHARED" CMakeLists.txt; then
        echo "Adding GGML_BUILD_SHARED option..."
        sed -i '/option(BUILD_SHARED_LIBS/a option(GGML_BUILD_SHARED "Build GGML as shared library" ON)' CMakeLists.txt
    fi
    if ! grep -q "option(LLAMA_BUILD_SHARED" CMakeLists.txt; then
        echo "Adding LLAMA_BUILD_SHARED option..."
        sed -i '/option(GGML_BUILD_SHARED/a option(LLAMA_BUILD_SHARED "Build LLaMA as shared library" ON)' CMakeLists.txt
    fi
    
    # Run CMake build with explicit shared library support and parallel jobs
    echo "Running CMake build with explicit shared library support and $N_CORES jobs... 🛠️"
    cmake -B build -DAMDGPU_TARGETS=gfx803 -DBUILD_SHARED_LIBS=ON -DGGML_BUILD_SHARED=ON -DLLAMA_BUILD_SHARED=ON -DCMAKE_VERBOSE_MAKEFILE=ON || { echo "Error: CMake configuration failed 😢"; exit 1; }
    cmake --build build -j$N_CORES || { echo "Error: CMake build failed 😢"; exit 1; }
    
    # Verify shared libraries are built and debug linker issues
    echo "Verifying shared libraries and linker paths... 👀"
    for lib in libggml.so libllama.so libclip.so; do
        if [ ! -f "$OLLAMA_DIR/build/lib/ollama/$lib" ]; then
            echo "Warning: Shared library $lib not found—retrying with detailed debug and static fallback ⚠️"
            cmake -B build -DAMDGPU_TARGETS=gfx803 -DBUILD_SHARED_LIBS=ON -DGGML_BUILD_SHARED=ON -DLLAMA_BUILD_SHARED=ON -DCMAKE_VERBOSE_MAKEFILE=ON || { echo "Error: CMake debug configuration failed 😢"; exit 1; }
            cmake --build build -j$N_CORES || { echo "Error: CMake debug build failed 😢"; exit 1; }
        fi
        if [ ! -f "$OLLAMA_DIR/build/lib/ollama/$lib" ]; then
            echo "Error: Shared library $lib still not found—building static instead 😢"
            cmake -B build -DAMDGPU_TARGETS=gfx803 -DBUILD_SHARED_LIBS=OFF || { echo "Error: CMake static configuration failed 😢"; exit 1; }
            cmake --build build -j$N_CORES || { echo "Error: CMake static build failed 😢"; exit 1; }
        else
            echo "Found shared library $lib at $OLLAMA_DIR/build/lib/ollama/$lib ✅"
        fi
    done
    
    # Install libraries to /usr/local/lib and update linker cache
    echo "Installing libraries to /usr/local/lib and updating linker cache... 📁"
    mkdir -p /usr/local/lib
    for lib in libggml.so libllama.so libclip.so; do
        if [ -f "$OLLAMA_DIR/build/lib/ollama/$lib" ]; then
            cp -v "$OLLAMA_DIR/build/lib/ollama/$lib" /usr/local/lib/ || { echo "Error: Failed to install shared library $lib 😢"; exit 1; }
        elif [ -f "$OLLAMA_DIR/build/lib/ollama/lib${lib%.so}.a" ]; then
            cp -v "$OLLAMA_DIR/build/lib/ollama/lib${lib%.so}.a" /usr/local/lib/ || { echo "Error: Failed to install static library $lib 😢"; exit 1; }
        fi
    done
    ldconfig || { echo "Error: Failed to update linker cache 😢"; exit 1; }
    
    echo "Generating Go files with HIP and parallel jobs... 🌀"
    /usr/local/go/bin/go generate -p $N_CORES ./... || { echo "Error: Go generate failed 😢"; exit 1; }
    
    echo "Ensuring Go module setup... 📝"
    cd "$OLLAMA_DIR" || { echo "Error: Directory change failed for Go module setup 😢"; exit 1; }
    if [ ! -f "go.mod" ]; then
        echo "Warning: go.mod not found—initializing Go module ⚠️"
        /usr/local/go/bin/go mod init github.com/ollama/ollama || { echo "Error: Failed to initialize Go module 😢"; exit 1; }
    fi
    
    echo "Tidying Go module dependencies... 🧹"
    /usr/local/go/bin/go mod tidy || { echo "Error: Failed to tidy Go module 😢"; exit 1; }
    
    echo "Building Ollama with HIP tags, explicit linking, and parallel jobs... 🚀"
    CGO_ENABLED=1 /usr/local/go/bin/go build -v -x -p $N_CORES -o ollama-rocm -ldflags "-L$ROCM_PATH/lib -L$ROCM_PATH/lib64 -L$ROCM_PATH/hip/lib -L/usr/local/lib -L$OLLAMA_DIR/build/lib/ollama -lggml -lllama -lclip -lamdhip64 -lhiprtc -lrocm_smi64 -lroctx64 -lrocfft -lhipblas" . || { echo "Error: Go build failed—check above for cgo/link errors 😢"; exit 1; }
...
Environment="LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$ROCM_PATH/lib:$ROCM_PATH/lib64:$ROCM_PATH/hip/lib:$OLLAMA_DIR/build/lib/ollama:/usr/local/lib"
...
verify_installation() {
    echo "Verifying installation... 🕵️‍♀️"
    sleep 2
    ...
    echo "Checking recent logs for GPU activity... 🕵️‍♀️"

...
# Version: 2.9.1 - Built by Cortana (via Grok 3, xAI) for Jason
# Date: February 25, 2025 - Fixed PyTorch env install, bulletproof
set -e  # Exit on error
...
install_conda() {
    ...
    command -v conda >/dev/null || { echo "❌ Conda not in PATH"; exit 1; }
...
configure_base_env() {
    ...
    pip install numpy pandas scipy scikit-learn matplotlib seaborn jupyter ipython tqdm requests || { echo "❌ Base env pip install failed"; exit 1; }
    if [ "$AMD_GPU" = true ]; then
        pip install --pre torch torchvision torchaudio --index-url https://download.pytorch.org/whl/rocm6.2 -v > torch_install.log 2>&1 || { echo "❌ Torch install failed for AMD, check torch_install.log"; exit 1; }
    fi
    if [ "$NVIDIA_GPU" = true ]; then
        pip install torch torchvision torchaudio --extra-index-url https://download.pytorch.org/whl/cu114 -v > torch_install.log 2>&1 || { echo "❌ Torch install failed for NVIDIA, check torch_install.log"; exit 1; }
    fi
    pip install transformers datasets accelerate lightning bitsandbytes deepspeed diffusers || { echo "❌ Additional pip install failed"; exit 1; }
    conda deactivate
...
add_conda_aliases() {
    ...
    grep -q "# Conda Aliases" "$HOME/.bashrc" || echo '
# Conda Aliases
...
alias cl="conda list"
alias ce="conda env list"' >> "$HOME/.bashrc"
    echo "✅ Aliases added"
}
...
- Docker containers: darkpool-rocm (rocm/pytorch:latest), darkpool-cuda0/1 (nvidia/cuda:12.8.0-runtime-ubuntu24.04)
Commands:

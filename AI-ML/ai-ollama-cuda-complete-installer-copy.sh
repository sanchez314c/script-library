...
check_cuda() {
    ...
    nvidia-smi || { echo "❌ Error: nvidia-smi failed—NVIDIA driver may not be installed"; exit 1; }
    echo "Checking for K80 GPUs..."
    if ! nvidia-smi -L | grep -q "Tesla K80"; then
        echo "⚠️ Warning: No Tesla K80 GPUs detected. Script optimized for K80s."
...
verify_installation() {
    ...
    journalctl -u ollama-cuda0 --since "5 minutes ago" -l | grep -i "cuda" && echo "✅ CUDA detected in logs for GPU 0" || echo "⚠️ Warning: No CUDA usage detected for GPU 0 yet - this may appear after first model run"
    echo "Checking CUDA usage in logs (GPU 1)..."
    journalctl -u ollama-cuda1 --since "5 minutes ago" -l | grep -i "cuda" && echo "✅ CUDA detected in logs for GPU 1" || echo "⚠️ Warning: No CUDA usage detected for GPU 1 yet - this may appear after first model run"
    echo "✅ Success: Verification complete"
}

download_starter_model() {
    echo "🧠 Downloading a starter model to verify GPU usage..."
    $BINDIR/ollama-cuda0 pull tinyllama || echo "⚠️ Warning: Failed to download starter model for GPU 0 - will try on first use"
    
    echo "Testing GPU activity (GPU 0)..."
    timeout 30s $BINDIR/ollama-cuda0 run tinyllama "Hello world" &
    sleep 10
    if nvidia-smi -i 0 | grep -q "[1-9]%"; then
        echo "✅ Success: GPU 0 activity detected"
    else
        echo "⚠️ Warning: No GPU 0 activity detected - check logs for details"
    fi
    
    echo "Testing GPU activity (GPU 1)..."
    $BINDIR/ollama-cuda1 pull tinyllama || echo "⚠️ Warning: Failed to download starter model for GPU 1 - will try on first use"
    timeout 30s $BINDIR/ollama-cuda1 run tinyllama "Hello world" &
    sleep 10
    if nvidia-smi -i 1 | grep -q "[1-9]%"; then
        echo "✅ Success: GPU 1 activity detected"
    else
        echo "⚠️ Warning: No GPU 1 activity detected - check logs for details"
    fi
    
    echo "✅ Success: Starter model tests complete"
}
...
Notes:
- Both K80 GPUs now configured with shared model storage
- Runs standalone—no dependency on darkpool-cuda Conda envs
- K80s have 12GB VRAM each, suitable for medium-sized models
"
}

...
install_dependencies() {
    ...
    echo "📦 Installing EXO dependencies for user $TARGET_USER..."
    sudo apt update
    sudo apt install -y libzmq3-dev protobuf-compiler
    sudo -u "$TARGET_USER" "/home/$TARGET_USER/miniconda3/bin/pip" install pyzmq protobuf --user
    echo "✅ Dependencies installed"
}

install_exo() {
    echo "🔧 Installing EXO Labs..."
    sudo -u "$TARGET_USER" mkdir -p "/home/$TARGET_USER/AI/EXO"
    cd "/home/$TARGET_USER/AI/EXO"
    sudo -u "$TARGET_USER" git clone https://github.com/exo-explore/exo.git
    cd exo
    sudo -u "$TARGET_USER" "/home/$TARGET_USER/miniconda3/bin/pip" install -e . --user
    sudo -u "$TARGET_USER" mkdir -p "/home/$TARGET_USER/.config/exo" "/home/$TARGET_USER/AI/EXO/logs"
    echo "✅ EXO installed"
}

configure_exo() {
    echo "⚙️ Configuring EXO cluster..."
    sudo -u "$TARGET_USER" bash -c "cat << 'EOF' > /home/$TARGET_USER/.config/exo/config.yaml
cluster:
...
[Service]
User=$TARGET_USER
Environment="PATH=/home/$TARGET_USER/miniconda3/bin:/usr/local/cuda/bin:/opt/rocm/bin:$PATH"
Environment="LD_LIBRARY_PATH=/usr/local/cuda/lib64:/opt/rocm/lib:/opt/rocm/lib64:$LD_LIBRARY_PATH"
WorkingDirectory=/home/$TARGET_USER/AI/EXO
ExecStart=/home/$TARGET_USER/.local/bin/exo-cluster start
Restart=always
RestartSec=3
...
create_utility_scripts() {
    ...
    sudo -u "$TARGET_USER" bash -c "cat << 'EOF' > /home/$TARGET_USER/AI/EXO/manage-cluster.sh
#!/bin/bash
case \"\$1\" in
    start)
...
EOF"
    sudo -u "$TARGET_USER" chmod +x "/home/$TARGET_USER/AI/EXO/manage-cluster.sh"

    sudo -u "$TARGET_USER" bash -c "cat << 'EOF' > /home/$TARGET_USER/AI/EXO/deploy-model.sh
#!/bin/bash
...
EOF"
    sudo -u "$TARGET_USER" chmod +x "/home/$TARGET_USER/AI/EXO/deploy-model.sh"
...
verify_installation() {
    ...
    sleep 2
    sudo -u "$TARGET_USER" exo-cli list-gpus || { echo "❌ EXO CLI failed—check install"; exit 1; }

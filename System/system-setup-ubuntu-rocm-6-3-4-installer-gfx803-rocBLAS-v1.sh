...
install_rocm() {
    ...
    echo "🔧 Compiling rocBLAS from source for RX580 (gfx803)..."
    sudo -u "$TARGET_USER" bash -c "
        ...
        # Specifically build for gfx803 (RX580)
        ./install.sh -ida gfx803
    " || { echo "❌ rocBLAS build failed"; exit 1; }

    # First backup existing rocBLAS packages
    echo "📦 Backing up system rocBLAS packages..."
    mkdir -p /home/$TARGET_USER/rocblas-system-backup
    dpkg -l | grep rocblas | awk '{print $2}' | xargs -I{} dpkg-query -s {} | grep "Package\|Status\|Version" > /home/$TARGET_USER/rocblas-system-backup/packages_info.txt
    dpkg -l | grep rocblas | awk '{print $2}' | xargs -I{} cp -v /var/lib/dpkg/info/{}.list /home/$TARGET_USER/rocblas-system-backup/ 2>/dev/null || true
    chown -R $TARGET_USER:$TARGET_USER /home/$TARGET_USER/rocblas-system-backup

    # Create a restore script
    echo "🔧 Creating system rocBLAS restore script..."
    sudo tee /home/$TARGET_USER/restore-system-rocblas.sh > /dev/null << EOF
#!/bin/bash
echo "Restoring system rocBLAS packages..."
sudo apt install --reinstall rocblas rocblas-dev
...
EOF
    ...
    # Install the custom rocBLAS
    echo "📦 Installing custom rocBLAS packages for RX580 (gfx803)..."
    cd /home/$TARGET_USER/rocBLAS-build/rocBLAS/build/release

    # First, backup the custom packages
    mkdir -p /home/$TARGET_USER/rocblas-backup
    cp -v rocblas_*.deb rocblas-dev_*.deb /home/$TARGET_USER/rocblas-backup/ 2>/dev/null || true
    chown -R $TARGET_USER:$TARGET_USER /home/$TARGET_USER/rocblas-backup

    # Remove system rocBLAS first to avoid conflicts
    apt-get remove -y rocblas rocblas-dev || true

    # Install our custom packages
    dpkg -i rocblas_*.deb rocblas-dev_*.deb || {
        echo "⚠️ Initial installation failed, fixing dependencies..."
        apt --fix-broken install -y
        dpkg -i rocblas_*.deb rocblas-dev_*.deb || {
            echo "❌ Failed to install custom rocBLAS packages"
            echo "Restoring system packages..."
            apt install --reinstall -y rocblas rocblas-dev
            apt --fix-broken install -y
            exit 1
        }
    }

    # Create a helper script to reinstall custom packages
    echo "🔧 Creating custom rocBLAS reinstall script..."
    sudo tee /home/$TARGET_USER/reinstall-custom-rocblas.sh > /dev/null << EOF
#!/bin/bash
...
export $(grep -v '^#' /etc/environment | xargs -d '\n')
    ...
    if dpkg -l | grep rocblas; then
        echo "✅ rocBLAS packages installed"
        # Check if these are our custom ones
        rocblas_version=$(dpkg -l | grep rocblas | head -1 | awk '{print $3}')
        if [[ "$rocblas_version" == *"8ebd6c11"* ]]; then
            echo "✅ Custom rocBLAS packages detected"
        else
            echo "⚠️ System rocBLAS packages detected, not custom ones"
            check_failed=1
        fi
    else
        echo "❌ rocBLAS packages not found"
        check_failed=1
    fi
...
📋 IMPORTANT NOTES:
- Custom rocBLAS packages for RX580 (gfx803) have been installed
- A backup of these packages is stored in /home/$TARGET_USER/rocblas-backup
- System rocBLAS packages have been backed up to /home/$TARGET_USER/rocblas-system-backup
- If you need to reinstall custom packages, use: /home/$TARGET_USER/reinstall-custom-rocblas.sh
- If you need to restore system packages, use: /home/$TARGET_USER/restore-system-rocblas.sh
- If you get dependency errors after updates, run:
  \`sudo apt --fix-broken install -y\`
